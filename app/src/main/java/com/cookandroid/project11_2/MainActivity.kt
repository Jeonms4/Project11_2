...
class MainActivity : AppCompatActivity() {
    ...
    inner class MyGalleryAdapter(internal var context: Context) : BaseAdapter() {
        ...
        var posterName = arrayOf(
            "여인의 향기", "쥬라기 공원", "포레스트 검프", "사랑의 블랙홀",
            "혹성탈출", "아름다운 비행", "내이름은 칸", "해리포터",
            "마더", "킹콩을 들다",
        )
...
//                Toast.makeText(applicationContext, posterName[p0], Toast.LENGTH_SHORT).show()
                var toast = Toast(this@MainActivity)
                var toastView = View.inflate(this@MainActivity, R.layout.toast1, null)

                toastText = toastView.findViewById<View>(R.id.textview1) as TextView
                toastText!!.setText(posterName[p0]);
